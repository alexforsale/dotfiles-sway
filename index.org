...
** Dependencies
- Nerd Fonts :: Get them from [[https://www.nerdfonts.com/][here]].
- Polkit-Kde-Agent :: Polkit is used for controlling system-wide privileges.
- wl-clipboard :: provides =wl-copy= and =wl-paste= command.
...
#+begin_src conf
  ...
  set $foreground $color07
  set $background $color00
#+end_src
** Output
#+begin_src conf
  output * bg /usr/share/backgrounds/sway/Sway_Wallpaper_Blue_1920x1080.png fill
...
#+begin_src conf
  exec systemctl --user import-environment WAYLAND_DISPLAY DISPLAY XDG_CURRENT_DESKTOP SWAYSOCK I3SOCK XCURSOR_SIZE XCURSOR_THEME QT_QPA_PLATFORMTHEME SSH_AUTH_SOCK
  exec dbus-update-activation-environment --systemd --all &
  exec /usr/lib/polkit-kde-authentication-agent-1 &
  exec gnome-keyring-daemon --components=pkcs11,secrets,ssh -r -d &
  exec wl-paste --type text --watch cliphist store &
  exec wl-paste --type image --watch cliphist store &
  exec /usr/bin/kdeconnectd
  exec mako
  exec $terminal --class ncmpcpp -e ncmpcpp
#+end_src
** Colors
#+begin_src conf
...
#+end_src
Using ~Alt+Print~ key will capture selected area, save it to =~/Pictures/Screenshots/= and also add it to the copy buffer.
#+begin_src conf
  bindsym Mod1+Print exec grimshot --notify savecopy area "${HOME}/Pictures/Screenshots/grimshot-$(date +%Y-%m-%d_%H_%M_%S).png"
#+end_src
Also set it to ~$mod+Shift+s~ simulating the windows keybinding (but save it to clipboard).
#+begin_src conf
  bindsym $mod+Shift+S exec grimshot --notify savecopy area "${HOME}/Pictures/Screenshots/grimshot-$(date +%Y-%m-%d_%H_%M_%S).png"
#+end_src
*** Browser
#+begin_src conf
...
#+begin_src conf
  bar {
      ...
      wrap_scroll yes
  	mode dock
  	font pango:JetBrainsMonoNL Nerd Font Mono 6 
  	tray_output *
  	icon_theme Papirus
      # When the status_command prints a new line to stdout, swaybar updates.
      # The default just shows the current date and time.
      ...
      colors {
          ...
          focused_workspace  $color02  $color03    $color06
          inactive_workspace $color01  $color01    $color06
          active_workspace   $color00  $color00    $color01
          urgent_workspace   $color01  $color01    $color11
          binding_mode       $color01  $color01    $color14
      }
  }
#+end_src
...
#+begin_src conf
  ...
  for_window [app_id="com.saivert.pwvucontrol"] floating enable; resize set 600 100; move position center
#+end_src
*** anki
#+begin_src conf
...
*** cheese
#+begin_src conf
  for_window [app_id="org.gnome.Cheese"] floating enable; resize set 680 680; move position center
#+end_src
*** easyeffects
#+begin_src conf
  for_window [class="(?i)easyeffects"] floating enable resize set 680 680 move position center
#+end_src
...
*** ncmpcpp
#+begin_src conf
  for_window [class="(?i)ncmpcpp"] floating enable; move scratchpad
#+end_src
*** OBS
#+begin_src conf
  for_window [class="(?i)com.obsproject.studio" title="(?i)auto-configuration-wizard"] floating enable resize set 680 680 move position center
#+end_src
*** polkit
#+begin_src conf
...
#+begin_src css
  ...
  @define-color color04 #434c5e;
  @define-color color05 #d8dee9;
  @define-color color06 #e5e9f0;
  @define-color color07 #eceff4;
  @define-color color08 #8fbcbb;
  @define-color color09 #88c0d0;
  @define-color color010 #81a1c1;
  @define-color color011 #5e81ac;
  @define-color color012 #bf616a;
  @define-color color013 #d08770;
  @define-color color014 #ebcb8b;
  @define-color color015 #a3be8c;
  @define-color color016 #b48ead;

  @define-color background-alpha rgba(46, 52, 64, 0.60);
...
** config
:PROPERTIES:
:header-args: :tangle ./.config/mako/config :mkdirp t
:END:
...
#+begin_src conf
  ...
  icon-path=/usr/share/icons/breeze:/usr/share/icons/Papirus-Dark
  anchor=top-right
#+end_src
